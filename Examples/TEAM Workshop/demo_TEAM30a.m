...
I = 2045.175*sqrt(2); %stator current (peak)
...
statorConductors{1} = Surfaces.get('A');
statorConductors{6} = Surfaces.get('Cminus');
statorConductors{5} = Surfaces.get('B');
statorConductors{4} = Surfaces.get('Aminus');
statorConductors{3} = Surfaces.get('C');
statorConductors{2} = Surfaces.get('Bminus');

%other elements in non-air domains
statorIron = Surfaces.get('StatorSteel');
rotorIron = Surfaces.get('RotorSteel');
rotorCoat = Surfaces.get('RotorAluminum');
...
%air-gap elements
el_ag = [Surfaces.get('AG_rotor') Surfaces.get('AG_stator');];
%el_ag = Surfaces.get('AG_stator');

%initializing mesh struct
msh = inittri(p, t);
msh.rotel = [rotorIron rotorCoat];
%msh.rotel = [rotorIron rotorCoat Surfaces.get('AG_rotor')];
msh.bandData = initializeBandData(msh, [], msh.rotel, msh.t(:, el_ag));
...
msh_fill(msh, Surfaces.get('AG_coils'), 'y');
msh_plot(msh, n_Dirichlet, 'ko');
msh_triplot(msh, el_ag, 'm');
...
%induced back-emfs per coil-side (= not per turn)
Es_coil = 1i*2*pi*fs*JC'*As / sqrt(2); %rms value desired --> division by sqrt(2)
...
%torque
T = compute_Torque_simple(As, msh, 3.0e-2, 3.2e-2, el_ag); %rms value returned by default
...
P_coat = P_coat/2; P_iron = P_iron/2; %rms value desired --> division by 2
...
try
    ...
    figure(3);
    plot(Data_Comsol(:,1), Data_Comsol(:, 10), 'rv');
    legend('Matlab', 'Comsol');
    drawnow;
    
    figure(5);
    subplot(2,1,1);
    plot(Data_Comsol(:,1), Data_Comsol(:,8), 'rv');
    legend('Matlab', 'Comsol', 'Location', 'SouthEast');
    subplot(2,1,2);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% performing time-stepping analysis
...
Asamples = zeros(Np, Nsamples); Asamples(:,1) = real(As(:,kw));
...
for ksample = 2:Nsamples
    ...
    FL = JC*L*[cos(theta); cos(theta - 2*pi/3);  cos(theta-4*pi/3)]*I + ...
...
figure(7); clf;
Tsamples = compute_Torque_simple(Asamples, msh, 3.0e-2, 3.2e-2, [], wms(kw)*tsamples);
