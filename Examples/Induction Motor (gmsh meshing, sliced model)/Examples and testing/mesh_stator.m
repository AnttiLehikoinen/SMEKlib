...
tol_ag = 1.0e-3; %airgap
tol_core1 = 1e-3; %slot opening area
tol_core2 = 5e-3; %slot sides
...
gw.addPcws('line', Xin, Xmid, -3, 'linename', 'n_cl', ...
    'line', Xmid, Xout, -2, 'linename', 'n_cl', ...
    'arc', [0;0], Xout, D*Xout, tol_out, 'linename', 'n_dir', ...
    'line', D*Xout, D*Xmid, -2, 'linename', 'n_ccl', ...
    'line', D*Xmid, D*Xin, -3, 'linename', 'n_ccl', ...
...
SC = cell(1, dim.Qs/symmetrySectors);
for k = 1:size(SC,2);
    SC{k} = [Surfaces_s.get('Layer1') Surfaces_s.get('Layer2')] + (k-1)*Ne_s_orig;
end
